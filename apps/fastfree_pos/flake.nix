{
  description = "FastFree POS — Container image for Point of Sale frontend SPA";
  ...
  outputs = { self, nixpkgs }:
    let
      ...
      meta = {
        description = "FastFree POS — Nix-built container image";
        platforms = [ "x86_64-linux" ];
      };

      spa-app = pkgs.stdenv.mkDerivation {
        pname = "fastfree-pos-spa";
        version = "1.0.0";
        src = ../../.;
        ...
        pnpmDeps = pkgs.fetchPnpmDeps {
          ...
          inherit pnpm;
          fetcherVersion = 3;
          hash = "sha256-QmE8DsFG3VqEqrUSP1FjjUapKx0w1hFylgh1ZywKxUc=";
        };
      ...
      frontend-image = pkgs.dockerTools.streamLayeredImage {
        ...
        config = {
          Labels = {
            ...
            "org.opencontainers.image.description" = "FastFree POS";
            "org.opencontainers.image.title" = "fastfree_pos";
          };
          Cmd = [ "${pkgs.caddy}/bin/caddy" "run" "--config" "/etc/caddy/Caddyfile" ];
          ExposedPorts = { "9003/tcp" = {}; };
        };
      };
