...
import type { UpdateConfig, UpdateMessages, UpdateSource, UpdateStatus } from './types'
...
function asFunction(value: unknown): ((...args: never[]) => unknown) | null {
...
function isNativePlugin(value: unknown): value is NativeUpdatePlugin {
  ...
  return typeof rec.getAppUpdateInfo === 'function' && typeof rec.startFlexibleUpdate === 'function'
}
...
export function resolveUpdateMessages(): UpdateMessages {
  return resolveUpdateLocale() === 'en' ? updateMessagesEn.update : updateMessagesAr.update
}
...
function resolveConfig(input?: Partial<UpdateConfig>): UpdateConfig {
  ...
  if (!Number.isFinite(merged.dismissCooldownDays) || merged.dismissCooldownDays <= 0) {
...
async function isNative(): Promise<boolean> {
  try {
    ...
    const mod: unknown = await import(/* @vite-ignore */ CAPACITOR_CORE_SPEC)
...
async function loadNativePlugin(): Promise<NativeUpdatePlugin | null> {
  try {
    ...
    const candidates: unknown[] = rec ? [rec.AppUpdate, rec.default, rec] : []
...
async function loadHostSwBoot(): Promise<HostSwBoot | null> {
  try {
    ...
    if (check) {
      out.checkForUpdate = check
    }
    const force = asFunction(rec.forceSWUpdate)
    if (force) {
      out.forceSWUpdate = force
    }
    return out
  } catch {
    return null
...
async function checkNative(): Promise<UpdateStatus> {
  try {
    ...
    const current = typeof info.currentVersionCode === 'number' ? info.currentVersionCode : null
    const availableCode =
      typeof info.availableVersionCode === 'number' ? info.availableVersionCode : null
