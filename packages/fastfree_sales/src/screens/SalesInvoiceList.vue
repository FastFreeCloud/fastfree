  ...
  <div class="q-pa-md">
    <q-card flat bordered>
      ...
      <DataTableBottom
        ...
        :rows-number="filteredInvoices.length"
        :width="tableWidth"
...
import {
  submitSalesInvoice as apiSubmitSalesInvoice,
  cancelSalesInvoice as apiCancelSalesInvoice,
  deleteSalesInvoice as apiDeleteSalesInvoice,
...
const columns = computed(() => [
  ...
  { name: 'due_date', label: t('sales.dueDate'), field: 'due_date', sortable: true },
  {
    ...
    field: 'grand_total',
    sortable: true,
    format: (v: number | null | undefined) => formatNumber(v ?? 0),
  },
...
const filteredInvoices = computed(() => {
  ...
  if (term === '') return store.salesInvoices
  return store.salesInvoices.filter((row: SalesInvoice) => {
    const haystack = [
      row.name,
      row.customer,
      row.customer_name,
...
const pagesNumber = computed(
  () => Math.ceil(filteredInvoices.value.length / pagination.value.rowsPerPage) || 1,
)
const bottomSummary = computed(() => `${filteredInvoices.value.length} ${t('sales.salesInvoices')}`)
const bottomTotal = computed(() =>
  formatNumber(filteredInvoices.value.reduce((s, i) => s + (i.grand_total ?? 0), 0)),
)

function handlePrint() {
  printTable({
    title: t('sales.salesInvoices'),
    columns: exportColumns.value as unknown as PrintColumn[],
    rows: filteredInvoices.value as unknown as Record<string, unknown>[],
    total: { label: t('sales.total'), value: filteredInvoices.value.length },
  })
}
...
async function confirmSubmitInvoice() {
  ...
  try {
    const result = await apiSubmitSalesInvoice(name)
    if (!result.success) {
...
async function confirmCancelInvoice() {
  ...
  try {
    const result = await apiCancelSalesInvoice(name)
    if (!result.success) {
      $q.notify({ type: 'negative', message: result.error?.message ?? t('common.error') })
      return
    }
...
async function confirmDeleteInvoice() {
  ...
  try {
    const result = await apiDeleteSalesInvoice(name)
    if (!result.success) {
      $q.notify({ type: 'negative', message: result.error?.message ?? t('common.error') })
      return
    }
